...
{
    [Serializable]
    public sealed class IonCannonProgram : List<IonCannonBehavior>, IRomSerializable
    {
        public IonCannonProgram()
            : base()
        {}

        private IonCannonProgram( RomSerializationInfo si, StreamingContext context )
            : base()
        {
        ...
        public void GetObjectData( RomSerializationInfo si, StreamingContext context )
        {
            ...
            {
                si.AddValue( $"Command{count++}", command );
            }
        }
    }
}
